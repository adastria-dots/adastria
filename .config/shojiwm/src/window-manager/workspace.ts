...
export class Workspace {
  ...
  // Hyprland's config always runs a column-based tiling layout — there's no
  // per-workspace floating-mode toggle bound in the source config, unlike the
  // stock default config's Super+S. Every workspace starts tiled here to
  // match that.
  public isTiled = true;
  ...
  // Bump the visible column count for this workspace by `delta`, clamped to
  // [1, MAX_COLUMNS].
  public bumpColumns(delta: number): number {
    const current = this.columnOverride ?? this.resolveMaxColumns(this.monitor);
    const next = clamp(current + delta, 1, MAX_COLUMNS);
  ...
  public applyLayout(options: LayoutOptions = {}) {
    ...
    // Center the row when it's narrower than the viewport (e.g. strict mode
    // with fewer windows than columns) instead of leaving it
  ...
  private tileWidthForWindow(
    ...
    // Monocle: fill the full viewport width, unclamped.
  ...
  // Each tile is one column wide, sized so `visibleColumnCount()` columns
  // (with TILE_GAP between them) exactly fill the viewport.
