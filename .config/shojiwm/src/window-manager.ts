...
// Column count for the scrollumns layout: a flat number for every monitor,
// or a table keyed by output name. Set via HybridWindowManager.configureScrollumns().
type ScrollumnsColumnConfig = number | Record<string, number>;
const SCROLLUMNS_MAX_COLUMNS = 6;
...
export class HybridWindowManager {
  ...
  private scrollumnsMaxColumns: ScrollumnsColumnConfig = 1;
  ...
  // scrollumns: per-monitor column count (flat number or table keyed by
  // output name), ported from utils/layout.lua's `Scrollumns.register(max_cols)`.
  public configureScrollumns(maxColumns: ScrollumnsColumnConfig): void {
    this.scrollumnsMaxColumns = maxColumns;
  }

  private resolveMaxColumns(monitor: string): number {
    if (typeof this.scrollumnsMaxColumns === "object") {
      return this.scrollumnsMaxColumns[monitor] ?? 1;
    }
    return this.scrollumnsMaxColumns;
  }
  ...
  // scrollumns column controls, ported from utils/layout.lua's bump/toggle_strict/
  // reset/toggle_monocle_or_maximize, operating on the active monitor's current
  // workspace (mirrors Lua's hl.get_active_workspace()).
  public bumpScrollumnsColumns(delta: number): number | undefined {
    const workspace = this.getCurrentWorkspace();
    if (!workspace) {
      return undefined;
    }
  ...
  public toggleScrollumnsStrict(): boolean | undefined {
    const workspace = this.getCurrentWorkspace();
    if (!workspace) {
      return undefined;
    }
  ...
  public resetScrollumnsLayout(): void {
    const workspace = this.getCurrentWorkspace();
    if (!workspace) {
      return;
    }
  ...
  // SUPER+F: monocle within scrollumns when the workspace is tiled, otherwise
  // fall back to the WM's normal maximize toggle (mirrors toggle_monocle_or_maximize).
  ...
  // T.mw(i): move the focused window to an absolute local workspace number on
  // its current monitor, as opposed to moveFocusedWindowToWorkspace's
  // relative +/-1 step. Ported from tiling.lua's Tiling.mw.
  ...
  // T.sw(i): swap every window between the active workspace and a target
  // local workspace number. Ported from tiling.lua's Tiling.sw — the Lua
  // version stages the swap through a temp special workspace to avoid
  // Hyprland dispatcher ordering issues; not needed here since both Workspace
  // objects are mutated directly and synchronously.
...
export class Workspace {
  ...
  private readonly resolveMaxColumns: (monitor: string) => number;
  private columnOverride: number | null = null;
  private strictColumns = false;
  private readonly monocleWindowIds = new Set<string>();
  private readonly forcedFloatWindowIds = new Set<string>();
  ...
  // Hyprland's config always runs the scrollumns layout (general.layout =
  // "lua:scrollumns") — there's no per-workspace floating-mode toggle bound
  // in the source config, unlike the stock default config's Super+S. Every
  // workspace starts tiled here to match that.
  public isTiled = true;
  ...
  public constructor(
    ...
    resolveMaxColumns: (monitor: string) => number,
  ) {
    this.index = index;
    ...
    this.resolveMaxColumns = resolveMaxColumns;
  }

  // scrollumns: bump the visible column count for this workspace by `delta`,
  // clamped to [1, SCROLLUMNS_MAX_COLUMNS]. Ported from layout.lua's bump().
  public bumpColumns(delta: number): number {
    const current = this.columnOverride ?? this.resolveMaxColumns(this.monitor);
    const next = clamp(current + delta, 1, SCROLLUMNS_MAX_COLUMNS);
    this.columnOverride = next;
    return next;
  }
  ...
  public resetColumnLayout(): void {
    ...
    this.monocleWindowIds.clear();
  }

  // Returns the new monocle state, or undefined if there's no active window
  // to toggle it for.
  public toggleMonocleForActiveWindow(): boolean | undefined {
    const window = this.activeWindow(this.tileableWindows());
    if (!window) {
      return undefined;
    }
    const next = !this.monocleWindowIds.has(window.id);
    if (next) {
      this.monocleWindowIds.add(window.id);
    } else {
      this.monocleWindowIds.delete(window.id);
    }
    return next;
  }

  // Number of columns actually shown: exactly `cols` in strict mode (columns
  // shrink to fit even with fewer windows open), otherwise capped at however
  // many tileable windows exist. Ported from layout.lua's `visible_cols`.
  private visibleColumnCount(): number {
    const cols = this.columnOverride ?? this.resolveMaxColumns(this.monitor);
    if (this.strictColumns) {
      return cols;
    }
    return Math.max(1, Math.min(this.tileableWindows().length, cols));
  }
  ...
  public applyLayout(options: LayoutOptions = {}) {
    ...
    // scrollumns: center the row when it's narrower than the viewport (e.g.
    // strict mode with fewer windows than columns) instead of leaving it
    // flush left. clampScrollOffset already forces scrollOffset to 0 in this
    // case. Ported from layout.lua's `center_shift`.
  ...
  public shouldTile(window: WaylandWindow): boolean {
    return (
      !this.forcedFloatWindowIds.has(window.id) &&
      window.isResizable() &&
      !window.isTransient()
    );
  }

  // Super+V: force the active window to float regardless of its natural
  // resizable/transient classification, or return it to normal tiling.
  // Hyprland's window.float({action="toggle"}) has no stock equivalent
  // here — shouldTile() above is otherwise a pure function of window
  // properties, with no per-window manual override.
  public toggleForcedFloatForActiveWindow(): boolean | undefined {
    const window = this.activeWindow();
    if (!window) {
      return undefined;
    }
    const next = !this.forcedFloatWindowIds.has(window.id);
    if (next) {
      this.forcedFloatWindowIds.add(window.id);
    } else {
      this.forcedFloatWindowIds.delete(window.id);
    }
    this.syncWindowVisibleOutputs(window);
    if (next) {
      // Going floating: seed a floating rect from its current tiled rect.
  ...
  private tileWidthForWindow(
    ...
    // scrollumns monocle: fill the full viewport width, unclamped, same as
    // layout.lua's `widths[i] = (addr and monocle[addr]) and ctx.area.w or col_w`.
    if (this.monocleWindowIds.has(window.id)) {
  ...
  // scrollumns: each tile is one column wide, sized so `visibleColumnCount()`
  // columns (with TILE_GAP between them) exactly fill the viewport. Ported
  // from layout.lua's `col_w = ctx.area.w / visible_cols`.
  private defaultTileWidth(viewportRect: ManagedWindowRect): number {
    const visibleCols = this.visibleColumnCount();
    const totalGap = (visibleCols - 1) * TILE_GAP;
    return Math.max(
      TILE_MIN_WIDTH,
      (read(viewportRect.width) - totalGap) / visibleCols,
    );
  }
